...
def main_summarization(max_budget, diffusion_models, weighting_schemes, 
    ...
    for name_id in all_name_ids:     
        for algorithm in algorithms:
            ...
            filename_with_path = results_folder+os.sep+'results'+name_id+os.sep+'runtime_files'+os.sep+filename
    ...
    output_df['ratio_with_celfpp'] = round(output_df['celfpp'] / output_df['ccelfpp1'],2)
    ...
    output_df.to_csv(results_folder+os.sep+'summary_runtimes.csv',index=False,header=True,sep=',')
    ...
    for name_id in all_name_ids:     
        for algorithm in algorithms:
            ...
            filename_with_path = results_folder+os.sep+'results'+name_id+os.sep+'pickle_files'+os.sep+filename
            ...
            if os.path.exists(filename_with_path):
                ...
                output_dict[algorithm] += [round(np.max([ item for i,item in enumerate(results_dict['exp_influence']) if i in indices]),2)]
    ...
    output_df['ratio_with_celfpp'] = round(output_df['ccelfpp1'] / output_df['celfpp'],2)
    output_df['ratio_with_cofim'] = round(output_df['ccelfpp1'] / output_df['cofim'],2)
    output_df.loc['average'] = round(output_df.mean(),2)
    
    "saving output_df as a csv file"
    output_df.to_csv(results_folder+os.sep+'summary_max_exp_influences.csv',index=False,header=True,sep=',')
    print(output_df.to_latex(index=False))