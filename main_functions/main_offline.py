...
"""
def main_offline(name_id, graph_type, max_budget, algorithms, heuristics, diffusion_models, 
                 is_graph_already_weighted, weighting_schemes, n_sim, communities, community_methods, 
    ...
    tmp = [ [network], weighting_schemes, algorithms, heuristics,[max_budget], diffusion_models ]
    tmp += [ [n_sim], [name_id] ]
    ...
    inputs1 = []
    for inpt in inputs:
        if inpt[2] in ['celfpp','ccelfpp1','genetic','ris','cris','cofim']  and inpt[3] == '':
            inputs1.append(inpt)
    ...
    if 'ccelfpp1' in algorithms:    
        ...
        if not os.path.exists('.'+os.sep+results_folder+os.sep+'results'+network.name+os.sep+'all_community_results'):
            os.makedirs('.'+os.sep+results_folder+os.sep+'results'+network.name+os.sep+'all_community_results')
        for folder_name in sorted(os.listdir('.'+os.sep+results_folder)):
            if network.name+'_community' in folder_name:
                #print(folder_name)
                shutil.move('.'+os.sep+results_folder+os.sep+folder_name,'.'+os.sep+results_folder+os.sep+'results'+network.name+os.sep+'all_community_results')
