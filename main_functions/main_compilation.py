...
def main_compilation(name_id, graph_type, max_budget, diffusion_models, weighting_schemes, 
    ...
    indices = sorted(list(set([0,1] + list(range(interval,max_budget+1,interval)))))
    output_df = output_df.loc[indices,:]
    
    "saving output_df as a csv file"
    output_df.to_csv(results_folder+os.sep+'plots'+'/sim_exp_influence_data_'+str(max_budget)+'.csv',index=False,header=True)
    ...
    legend_entries = ['CELF++','Community-IM','Genetic','CoFIM','Degree','Degree-Discount']
    ...
    ax.get_figure().savefig(results_folder+os.sep+'plots'+'/sim'+name_id+'_plot_'+str(max_budget)+'.eps')
    ax.get_figure().savefig(results_folder+os.sep+'plots'+'/sim'+name_id+'_plot_'+str(max_budget)+'.jpg')
    tikzplotlib.clean_figure()
    tikzplotlib.save(results_folder+os.sep+'plots'+'/sim'+name_id+'_plot_'+str(max_budget)+'.tex')