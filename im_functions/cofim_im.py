...
def cofim_im(network, weighting_scheme, budget, diffusion_model, n_sim=100, 
    ...
    cofim_folder_new = cofim_folder+'_copies'+os.sep+'cofim_'+network.name[1:]+'_'+diffusion_model+'_'+weighting_scheme 
    ...
    nx.write_edgelist(network, cofim_folder_new+os.sep+network.name[1:]+'.txt', delimiter='\t', data = False)
    ...
    try:
        ...
        pass
    with open(cofim_folder_new+os.sep+network.name[1:]+'_com.txt', 'a') as outfile:
        for community in communities:
    ...
    try:
        ...
        pass
    os.system(">>"+output_file+" ./CoFIM -data "+network_file+" -com "+network_com_file+" -gamma 3 -k "+str(budget))
    end = timeit.default_timer()
    runtime = end - start 
            
    "reading the output file"
    output = open(output_file).read().strip().split('time(s)')[1].strip().split('\n')[:-4]
    ...
    runtime_info['cofim'] = runtime_info['community_detection'] + runtime_info['cofimwc']
    ...
    if all_upto_budget == True:
        ...
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'cofim', 'n_sim':n_sim, \
        ...
        fstr = results_folder_pickle_files+os.sep+'output_cofim__%i__.pkl'%(budget)
        ...
        logging.info('Total time taken by CoFIM is '+str(round(end - start,2))+' seconds.') 
        
        return [[None]] + final_best_seed_sets, [0] + final_exp_influences,  runtime_info['cofim']
