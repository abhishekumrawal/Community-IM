...
import pickle
import json
from networkx.algorithms.community import is_partition, coverage, performance, modularity
import logging
...
def ccelfpp1_im(network, weighting_scheme, budget, diffusion_model, n_sim=100, 
                community_method='louvain', communities=[], community_size_threshold = 0.01,
    ...
    if communities == []:
        logging.info("Part 1: Detecting communities using "+community_method+" method.")
        ...
        logging.info('Time taken in community detection is '+str(round(endc - startc,2))+' seconds.') 
    else:
    ...
    coverage_score = coverage(network, communities)
    #performance_score = performance(network, communities)
    modularity_score = modularity(network, communities, weight='act_prob')
    
    logging.info('Number of communities detected/provided is '+str(num_communities)+'.')
    ...
    small_communities = [community for community in communities if len(community) < community_size_threshold* len(network.nodes)]
    large_communities = [community for community in communities if community not in small_communities]
    
    merged_small_communities = [item for sublist in small_communities for item in sublist]
    ...
    logging.info('Number of new communities detected/provided is '+str(len(new_communities))+'.')
    logging.info('Sizes of the new communities are as follwows.')
    logging.info(str([len(community) for community in new_communities]))
    logging.info('New communities form a valid partition: ' + str(is_partition(network, new_communities))+'.')  
    logging.info('Coverage of the new partition is ' + str(coverage(network, new_communities)))
    #logging.info('Performance of the new partition: ' + str(performance(network, new_communities))+'.')
    logging.info('Modularity of the new partition is ' + str(modularity(network, new_communities))+'.')
    ...
    for i,community in enumerate(communities):
        ...
        sub_network = network.subgraph(community).copy() # .copy() makes a subgraph with its own copy of the edge/node attributes
        sub_network.name = network.name+'_community_'+str(i)
        budget_for_greedy = min(len(sub_network.nodes),budget)
        best_seed_set, exp_influence, runtime = celfpp_im(sub_network, weighting_scheme, budget_for_greedy, diffusion_model, n_sim, all_upto_budget=True)
        runtime_info['celfpp'] += runtime
        logging.info(str(best_seed_set)) 
        logging.info(str(exp_influence))
        all_communities_results[i] = [list(range(0,budget_for_greedy+1)),best_seed_set,exp_influence]    
        endi = timeit.default_timer()
        logging.info('Time taken by Community-CELF++ for community '+ str(i)+' is '+str(round(endi - starti,2))+' seconds.')
    ...
    logging.info('Time taken in progressive budgeting is '+str(round(endk - startk,2))+' seconds.')
    end = timeit.default_timer()    
    logging.info('Total time taken by Community-CELF++ until now is'+' '+str(round(end - start,2))+' seconds.') 
    
    "saving runtime info"
    runtime_info['ccelfpp1'] = runtime_info['community_detection'] + runtime_info['celfpp'] + runtime_info['progressive_budgeting']
    ...
    if all_upto_budget == True:
        ...
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'ccelfpp1', 'n_sim':n_sim, \
                   'community_method':community_method, 'communities':communities, 'num_communities':num_communities, \
                       'sizes_communities':sizes_communities, 'is_valid_partition':is_valid_partition, \
                           'coverage_score':coverage_score, 'modularity_score':modularity_score, 'network_name':network.name, \
                               'best_seed_set':[[None]] + final_best_seed_sets, 'exp_influence':[0] + final_exp_influences}
     
        fstr = results_folder_pickle_files+os.sep+'output_ccelfpp1__%i__.pkl'%(budget)
        ...
        logging.info('Total time taken by Community-CELF++ is '+str(round(end - start,2))+' seconds.') 
        
        return [[None]] + final_best_seed_sets, [0] + final_exp_influences,  runtime_info['ccelfpp1']
        ...
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'ccelfpp1', 'n_sim':n_sim,\
                   'community_method':community_method, 'communities':communities, 'num_communities':num_communities, \
                       'sizes_communities':sizes_communities, 'is_valid_partition':is_valid_partition, \
                           'coverage_score':coverage_score, 'modularity_score':modularity_score, 'network_name':network.name, \
                               'best_seed_set':final_best_seed_set, 'exp_influence':final_exp_influence}
