...
def progressive_budgeting(all_communities_results, budget):
    ...
    for key in all_communities_exp_influences.keys():
        all_communities_marginal_gains[key] = [
            all_communities_exp_influences[key][i + 1]
            - all_communities_exp_influences[key][i]
    ...
    for k in range(1, budget + 1):
        ...
        current_marginal_gains = [
            all_communities_marginal_gains[key][0]
            for key in all_communities_marginal_gains.keys()
        ]
        ...
        all_communities_marginal_gains[chosen_community].remove(
            all_communities_marginal_gains[chosen_community][0]
        )
        ...
        for key in all_communities_budget_allocation.keys():
            final_best_seed_set += all_communities_best_seed_sets[key][
                all_communities_budget_allocation[key]
            ]
