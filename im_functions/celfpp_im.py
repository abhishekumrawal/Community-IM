...
def celfpp_im(network, weighting_scheme, budget, diffusion_model, n_sim=100, all_upto_budget=True):
    ...
    celfpp_folder_new = celfpp_folder+'_copies'+os.sep+'celf++_'+network.name[1:]+'_'+diffusion_model+'_'+weighting_scheme 
    ...
    # Generating input for Goyal's software, i.e.
        - graph.inf : the actual weighted diretced graph
    """
    ...
    graph.to_csv(celfpp_folder_new+"/datasets/graph.inf",sep=" ",index=False,header=True)
    ...
    if diffusion_model == 'independent_cascade':
        config_test = ' '.join(config_test.split(' ')[0:17] + ['IC'+'\n\nprobGraphFile'] + config_test.split(' ')[18:])
    elif diffusion_model == 'linear_threshold':
        config_test = ' '.join(config_test.split(' ')[0:17] + ['LT'+'\n\nprobGraphFile'] + config_test.split(' ')[18:])
    
    "##### Changing the path to ProbGraphFile"
    config_test = ' '.join(config_test.split(' ')[0:19] + [celfpp_folder_new+'/datasets/graph.inf'+'\n\nmcruns'] + config_test.split(' ')[20:])
    
    "##### Setting the mcruns in the config_test file as n_sim"
    config_test = ' '.join(config_test.split(' ')[0:21] + [str(n_sim)+'\n\noutdir'] + config_test.split(' ')[22:])
    
    "##### Changing the path to outdir"
    config_test = ' '.join(config_test.split(' ')[0:23] + [celfpp_folder_new+'/output'+'\n\nbudget'] + config_test.split(' ')[24:])
    
    "##### Setting the budget in the config_test file as budget"
    config_test = ' '.join(config_test.split(' ')[0:25] + [str(budget)+'\n\n#'] + config_test.split(' ')[26:])
    ...
    if all_upto_budget == True:
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'celfpp', 'n_sim':n_sim, \
        ...
        fstr = results_folder_pickle_files+os.sep+'output_celfpp__%i__.pkl'%(budget)
        ...
        logging.info('Total time taken by CELF++ is '+' '+str(round(runtime,2))+' seconds.') 
                        
        return [[None]] + [best_seed_set[:k+1] for k,_ in enumerate(best_seed_set)], [0] + exp_influence, runtime
    
    else:
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'CELF++', 'n_sim':n_sim, \
