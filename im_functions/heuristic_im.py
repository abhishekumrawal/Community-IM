...
def heuristic_im(network, weighting_scheme, heuristic, budget, diffusion_model, n_sim=100, all_upto_budget=True):  
    ...
    runtime = end - start
    logging.info('Time taken by '+heuristic+ ' is '+str(round(runtime,2))+' seconds.')
    ...
    if all_upto_budget == True:
        ...
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':heuristic, 'n_sim':n_sim, \
                   'network_name':network.name, \
                       'best_seed_set':[[None]] + final_best_seed_sets, 'exp_influence':[0] + final_exp_influences}
     
        fstr = results_folder_pickle_files+os.sep+'output_'+heuristic+'__%i__.pkl'%(budget)
        ...
        logging.info('Total time taken by '+heuristic+' is '+str(round(end - start,2))+' seconds.')  
        
        return [[None]] + final_best_seed_sets, [0] + final_exp_influences,  runtime_info[heuristic]
